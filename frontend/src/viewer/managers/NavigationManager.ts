...
interface NavigationExamViewer {
	currentExam: {
		exam_info?: {
			sections?: NavSection[];
		};
	...
	getCurrentCategory: () => NavCategory | null;
...
class NavigationManager {
	private readonly examViewer: NavigationExamViewer;
	private readonly logger: NavLogger;

	constructor(examViewer: NavigationExamViewer) {
		this.examViewer = examViewer;
		const loggerFactory = window.Logger as { getLogger?: (tag: string) => NavLogger } | undefined;
	...
	calculateNextQuestionPosition(
		currentCategoryData: NavCategory,
		sections: NavSection[],
	...
	calculatePrevQuestionPosition(
		currentCategoryData: NavCategory,
		sections: NavSection[],
		_currentSection: NavSection | undefined
	): NavPosition | null {
		const { currentSectionIndex, currentQuestionIndex } = this.examViewer;
	...
	navigateToQuestion(direction: 'next' | 'prev'): boolean {
