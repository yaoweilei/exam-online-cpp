...
//   GET    /api/v1/community/{paperId}                         列表
//   POST   /api/v1/community/{paperId}/posts                   发帖 {title, body}
//   DELETE /api/v1/community/{paperId}/posts/{postId}          删除帖子（作者或 superAdmin）
//   POST   /api/v1/community/{paperId}/posts/{postId}/comments 评论 {body}
//   POST   /api/v1/community/{paperId}/posts/{postId}/like     切换点赞
//
...
void registerCommunityRoutes(const AppContext &ctx)
{
    ...
    // 评论
    app().registerHandler(
        "/api/v1/community/{1}/posts/{2}/comments",
        [ctx](const HttpRequestPtr &req,
              std::function<void(const HttpResponsePtr &)> &&callback,
              std::string paperId,
              std::string postId) {
    ...
    // 切换点赞
    app().registerHandler(
        "/api/v1/community/{1}/posts/{2}/like",
        [ctx](const HttpRequestPtr &req,
              std::function<void(const HttpResponsePtr &)> &&callback,
              std::string paperId,
              std::string postId) {
