...
Json::Value UserService::searchUsers(const std::string &query, std::size_t limit) const
{
    ...
    // Account operators usually paste an exact account, member number, phone,
    // or user id. Resolve that path before the fuzzy scan so a large profile
    // directory does not turn the most common lookup into thousands of file
    // reads.
    auto exactUser = repository_.findUserByLoginId(query);
    if (exactUser.isNull())
    {
        exactUser = repository_.findUserByEmail(query);
    }
    if (!exactUser.isNull())
    {
        out.append(buildUserView(exactUser));
        return out;
    }
    ...
    std::unordered_map<std::string, std::size_t> hitIndexes;
    const auto users = repository_.users();
    const auto addHit = [&](const Json::Value &user, const Json::Value &profile) {
        const auto userId = user.get("id", user.get("user_id", "")).asString();
        const auto score = scoreUserSearchMatch(user, profile, needle);
        if (userId.empty() || score <= 0)
        {
            return;
        }
        const auto existing = hitIndexes.find(userId);
        if (existing != hitIndexes.end())
        {
            hits[existing->second].score = (std::max)(hits[existing->second].score, score);
            return;
        }
        hitIndexes[userId] = hits.size();
        hits.push_back({score, normalizeSearchText(user.get("username", userId).asString()), user});
    };

    const Json::Value emptyProfile(Json::objectValue);
    for (const auto &userId : users.getMemberNames())
    {
        addHit(users[userId], emptyProfile);
    }

    const auto profileLimit = (std::max)(static_cast<std::size_t>(50), (std::min)(limit * 4, static_cast<std::size_t>(200)));
    for (const auto &profile : profileRepository_.searchByDisplayName(needle, profileLimit))
    {
        const auto userId = profile.get("user_id", "").asString();
        if (!userId.empty() && users.isMember(userId))
        {
            addHit(users[userId], profile);
        }
    }
    ...
    {
        out.append(buildUserView(hits[index].user));
    }
    return out;
}
...
Json::Value UserService::platformRoleTemplates() const
{
...
Json::Value UserService::previewRoleTemplate(const std::string &roleId, const Json::Value &payload) const
{
    const auto roles = repository_.roles();
    if (!roles.isMember(roleId)) throw common::AppException("ROLE_NOT_FOUND", "Role template not found", drogon::k404NotFound);
    const auto before = roles[roleId].get("permissions", Json::Value(Json::arrayValue));
    const auto after = payload.get("permissions", before);
    Json::Value added(Json::arrayValue), removed(Json::arrayValue), conflicts(Json::arrayValue);
    auto contains = [](const Json::Value &items, const std::string &value) { for (const auto &item : items) if (item.asString() == value) return true; return false; };
    for (const auto &item : after) if (!contains(before, item.asString())) added.append(item);
    for (const auto &item : before) if (!contains(after, item.asString())) removed.append(item);
    if (roleId == "superAdmin" && (!contains(after, "*") || after.size() != 1)) conflicts.append("超级管理员权限固定为 *，不能拆分或移除");
    if (contains(after, "*") && roleId != "superAdmin") conflicts.append("普通角色不能授予通配权限 *");
    Json::Value out(Json::objectValue);
    out["role_id"] = roleId; out["before"] = before; out["after"] = after;
    out["added"] = added; out["removed"] = removed; out["conflicts"] = conflicts;
    out["can_apply"] = conflicts.empty();
    return out;
}

Json::Value UserService::updateRoleTemplate(const std::string &roleId, const Json::Value &payload)
{
    const auto preview = previewRoleTemplate(roleId, payload);
    if (!preview["can_apply"].asBool()) throw common::AppException("PERMISSION_CONFLICT", "Role template has permission conflicts", drogon::k409Conflict);
    Json::Value out(Json::objectValue);
    out["template"] = repository_.updateRoleTemplate(roleId, payload);
    out["diff"] = preview;
    return out;
}

Json::Value UserService::resolvePlatformAccess(const Json::Value &access, const Json::Value &roles) const
{
    Json::Value effectiveRoles = access.get("roles", Json::Value(Json::arrayValue));
    Json::Value permissions(Json::arrayValue), activeGrants(Json::arrayValue), expiredGrants(Json::arrayValue), conflicts(Json::arrayValue);
    auto appendUnique = [](Json::Value &items, const std::string &value) { if (value.empty()) return; for (const auto &item : items) if (item.asString() == value) return; items.append(value); };
    for (const auto &role : effectiveRoles)
        for (const auto &permission : roles[role.asString()].get("permissions", Json::Value(Json::arrayValue))) appendUnique(permissions, permission.asString());
    const auto now = common::nowIso8601();
    for (const auto &grant : access.get("temporary_grants", Json::Value(Json::arrayValue)))
    {
        const auto expiresAt = grant.get("expires_at", "").asString();
        if (!expiresAt.empty() && expiresAt <= now) { expiredGrants.append(grant); continue; }
        activeGrants.append(grant);
        const auto roleId = grant.get("role_id", "").asString();
        const auto effect = grant.get("effect", "allow").asString();
        if (!roleId.empty() && effect != "deny")
        {
            appendUnique(effectiveRoles, roleId);
            for (const auto &permission : roles[roleId].get("permissions", Json::Value(Json::arrayValue)))
                appendUnique(permissions, permission.asString());
        }
        for (const auto &permission : grant.get("permissions", Json::Value(Json::arrayValue)))
        {
            if (effect == "deny")
            {
                for (Json::ArrayIndex i = 0; i < permissions.size(); ++i) if (permissions[i].asString() == permission.asString()) { permissions.removeIndex(i, nullptr); break; }
            }
            else appendUnique(permissions, permission.asString());
        }
    }
    bool superAdmin = false;
    for (const auto &role : effectiveRoles) if (role.asString() == "superAdmin") superAdmin = true;
    if (superAdmin && effectiveRoles.size() > 1) conflicts.append("超级管理员与其他角色重复，实际以超级管理员通配权限为准");
    Json::Value out(Json::objectValue);
    out["roles"] = effectiveRoles; out["permissions"] = permissions;
    out["active_temporary_grants"] = activeGrants; out["expired_temporary_grants"] = expiredGrants;
    out["conflicts"] = conflicts;
    return out;
}

Json::Value UserService::previewPlatformAccess(const std::string &userId, const Json::Value &payload) const
{
    const auto beforeAccess = repository_.platformAccess(userId);
    Json::Value afterAccess = beforeAccess;
    if (payload.isMember("roles")) afterAccess["roles"] = payload["roles"];
    if (payload.isMember("temporary_grants")) afterAccess["temporary_grants"] = payload["temporary_grants"];
    const auto roles = repository_.roles();
    const auto before = resolvePlatformAccess(beforeAccess, roles);
    const auto after = resolvePlatformAccess(afterAccess, roles);
    Json::Value out(Json::objectValue);
    out["user_id"] = userId; out["before"] = before; out["after"] = after;
    Json::Value added(Json::arrayValue), removed(Json::arrayValue);
    auto contains = [](const Json::Value &items, const std::string &value) { for (const auto &item : items) if (item.asString() == value) return true; return false; };
    for (const auto &item : after["permissions"]) if (!contains(before["permissions"], item.asString())) added.append(item);
    for (const auto &item : before["permissions"]) if (!contains(after["permissions"], item.asString())) removed.append(item);
    out["added_permissions"] = added; out["removed_permissions"] = removed; out["conflicts"] = after["conflicts"];
    return out;
}

Json::Value UserService::updatePlatformAccess(const std::string &actorId, const std::string &userId, const Json::Value &payload)
{
    const auto preview = previewPlatformAccess(userId, payload);
    const auto beforeRoles = preview["before"]["roles"];
    const auto afterRoles = preview["after"]["roles"];
    auto hasRole = [](const Json::Value &items, const std::string &role) { for (const auto &item : items) if (item.asString() == role) return true; return false; };
    if (actorId == userId && hasRole(beforeRoles, "superAdmin") != hasRole(afterRoles, "superAdmin"))
        throw common::AppException("SELF_SUPERADMIN_CHANGE_FORBIDDEN", "You cannot modify your own super administrator role", drogon::k403Forbidden);
    if (hasRole(beforeRoles, "superAdmin") && !hasRole(afterRoles, "superAdmin") && repository_.countUsersWithRole("superAdmin") <= 1)
        throw common::AppException("LAST_SUPERADMIN_PROTECTED", "The last super administrator cannot be removed", drogon::k409Conflict);
    Json::Value out(Json::objectValue);
    out["access"] = repository_.updatePlatformAccess(userId, payload);
    out["diff"] = preview;
    return out;
}
...
Json::Value UserService::permissions(const std::string &userId) const
{
    ...
    const auto platformAccess = resolvePlatformAccess(repository_.platformAccess(userId), repository_.roles());
    const auto roles = effectiveRoles(platformAccess["roles"], membership["roles"]);
    Json::Value out(Json::objectValue);
    ...
    out["platform_permissions"] = platformAccess["permissions"];
    out["temporary_grants"] = platformAccess["active_temporary_grants"];
    out["permission_conflicts"] = platformAccess["conflicts"];
...
Json::Value UserService::buildUserView(const Json::Value &user) const
{
    ...
    out["has_password"] = user.get("has_password", false).asBool();
    out["hasPassword"] = out["has_password"].asBool();
    out["wechat_bound"] = user.get("wechat_bound", false).asBool();
    out["wechatBound"] = out["wechat_bound"].asBool();
