...
bool ClassroomRepository::update(const std::string &classId, const Json::Value &patch)
{
    ...
    for (auto &c : doc["classrooms"])
    {
        ...
        {
            c["name"] = patch["name"].asString();
        }
        if (patch.isMember("description"))
        {
            c["description"] = patch["description"].asString();
        }
        if (patch.isMember("student_ids") && patch["student_ids"].isArray())
        {
            c["student_ids"] = patch["student_ids"];
        }
