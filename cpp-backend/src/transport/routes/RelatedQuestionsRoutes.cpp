...
{
// 同考点串题（功能 #17）路由
//   GET  /api/v2/related-questions?exam_id=X&question_id=Y&limit=10
//   GET  /api/v2/related-questions/stats    （调试用；返回索引规模）
//   POST /api/v2/related-questions/rebuild  （管理员：强制重建索引）
void registerRelatedQuestionsRoutes(const AppContext &ctx)
{
    app().registerHandler(
        "/api/v2/related-questions",
    ...
    app().registerHandler(
        "/api/v2/related-questions/stats",
        [ctx](const HttpRequestPtr &req,
              std::function<void(const HttpResponsePtr &)> &&callback) {
            handleRequest(req, std::move(callback), [&]() {
                requireFeature(*ctx.featureFlagService, "related_questions", "");
    ...
    app().registerHandler(
        "/api/v2/related-questions/rebuild",
        [ctx](const HttpRequestPtr &req,
              std::function<void(const HttpResponsePtr &)> &&callback) {
            handleRequest(req, std::move(callback), [&]() {
