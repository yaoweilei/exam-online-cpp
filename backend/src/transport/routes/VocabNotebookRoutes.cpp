...
//
//   GET    /api/v1/vocab-notebook/{userId}
//   POST   /api/v1/vocab-notebook/{userId}/words      body: {word, reading, note?, exam_id?, question_id?}
//   DELETE /api/v1/vocab-notebook/{userId}/words/{wordId}
//   PATCH  /api/v1/vocab-notebook/{userId}/words/{wordId}   body: {note}
void registerVocabNotebookRoutes(const AppContext &ctx)
{
    app().registerHandler(
        "/api/v1/vocab-notebook/{1}",
    ...
    app().registerHandler(
        "/api/v1/vocab-notebook/{1}/words/{2}",
        [ctx](const HttpRequestPtr &req,
              std::function<void(const HttpResponsePtr &)> &&callback,
              std::string userId,
              std::string wordId) {
