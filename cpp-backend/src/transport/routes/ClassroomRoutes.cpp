...
// 班级与作业 路由（业务功能 6）
//   班级：
//     POST   /api/v2/classrooms                          创建（teacher/orgAdmin/superAdmin）
//     GET    /api/v2/me/classrooms                       我的班级（学生或教师视角）
//     GET    /api/v2/classrooms/{id}                     详情（成员可见）
//     PATCH  /api/v2/classrooms/{id}                     更新 name/description/student_ids（owner+）
//     DELETE /api/v2/classrooms/{id}                     删除（owner+）
//     POST   /api/v2/classrooms/{id}/members             批量加学生（owner+）
//     DELETE /api/v2/classrooms/{id}/members/{userId}    踢出学生（owner+）
//   作业：
//     POST   /api/v2/classrooms/{id}/assignments         创建（owner+）
//     GET    /api/v2/classrooms/{id}/assignments         列表（成员可见）
//     PATCH  /api/v2/assignments/{id}                    更新（owner+；只校简单存在）
//     DELETE /api/v2/assignments/{id}                    删除（owner+）
//     GET    /api/v2/me/assignments                      我的作业（聚合所有班级）
// ---------------------------------------------------------------------------
...
void registerClassroomRoutes(const AppContext &ctx)
{
    ...
    // 创建作业
    app().registerHandler(
        "/api/v2/classrooms/{1}/assignments",
        [ctx](const HttpRequestPtr &req,
              std::function<void(const HttpResponsePtr &)> &&callback,
              std::string classId) {
            handleRequest(req, std::move(callback), [&]() {
