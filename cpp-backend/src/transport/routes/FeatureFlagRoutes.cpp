...
// ---------------------------------------------------------------------------
// 功能开关路由（横切基础设施）
//   GET    /api/v2/feature-flags/registry              所有可用 flag 元信息（公开）
//   GET    /api/v2/me/feature-flags                    当前登录用户的解析结果
//   PUT    /api/v2/me/feature-flags                    用户自助修改可覆盖项
//                                                      body: { "<key>": { enabled, lock? } | null }
//   PUT    /api/v2/admin/feature-flags/system          超管修改 system 默认（需 superAdmin 角色）
//   PUT    /api/v2/admin/feature-flags/orgs/{orgId}    组织 owner / 超管修改 org 覆盖
// ---------------------------------------------------------------------------
void registerFeatureFlagRoutes(const AppContext &ctx)
{
    // 注册表（公开，前端启动时也可拉一次以渲染管理界面）
    app().registerHandler(
        "/api/v2/feature-flags/registry",
    ...
    // 超管修改 system
    app().registerHandler(
        "/api/v2/admin/feature-flags/system",
        [ctx](const HttpRequestPtr &req,
              std::function<void(const HttpResponsePtr &)> &&callback) {
            handleRequest(req, std::move(callback), [&]() {
                const auto body = parseJsonBody(req);
    ...
    app().registerHandler(
        "/api/v2/admin/feature-flags/orgs/{1}",
        [ctx](const HttpRequestPtr &req,
              std::function<void(const HttpResponsePtr &)> &&callback,
              std::string orgId) {
            handleRequest(req, std::move(callback), [&]() {
