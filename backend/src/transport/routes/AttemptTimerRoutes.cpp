...
//   GET    /api/v1/timers/{userId}              查询当前计时；无返回 null
//   POST   /api/v1/timers/{userId}/start        启动/重置
//                                               body: { exam_id*, total_limit_seconds?, section_limits_seconds? }
//   POST   /api/v1/timers/{userId}/tick         心跳累加（前端定时器调用）
//                                               body: { exam_id*, section_index?, delta_seconds }
//                                               若 exam_id 不匹配返回 null（前端应重 start）
//   POST   /api/v1/timers/{userId}/finish       完成 / 放弃（清理）
// ---------------------------------------------------------------------------
