...
// ---------------------------------------------------------------------------
// 收藏夹/分类 路由（业务功能 8）
//   GET    /api/v2/bookmark-folders/{userId}                         列表
//   POST   /api/v2/bookmark-folders/{userId}                         创建 {name, color?}
//   PATCH  /api/v2/bookmark-folders/{userId}/{folderId}              改名/换色
//   DELETE /api/v2/bookmark-folders/{userId}/{folderId}              删除
//   POST   /api/v2/bookmark-folders/{userId}/{folderId}/exams        加试卷 {exam_id}
//   DELETE /api/v2/bookmark-folders/{userId}/{folderId}/exams/{ex}   移除试卷
// ---------------------------------------------------------------------------
...
void registerBookmarkFolderRoutes(const AppContext &ctx)
{
    ...
    // 加试卷到文件夹
    app().registerHandler(
        "/api/v2/bookmark-folders/{1}/{2}/exams",
        [ctx](const HttpRequestPtr &req,
              std::function<void(const HttpResponsePtr &)> &&callback,
              std::string userId,
              std::string folderId) {
    ...
    // 从文件夹移除试卷
    app().registerHandler(
        "/api/v2/bookmark-folders/{1}/{2}/exams/{3}",
        [ctx](const HttpRequestPtr &req,
              std::function<void(const HttpResponsePtr &)> &&callback,
              std::string userId,
              std::string folderId,
