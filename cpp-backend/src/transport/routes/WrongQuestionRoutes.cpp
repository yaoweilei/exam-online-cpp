...
// ---------------------------------------------------------------------------
// 错题本相关路由
//   GET    /api/v2/wrong-questions/{userId}                  错题列表（含筛选/分页/统计）
//   GET    /api/v2/wrong-questions/{userId}/summary          仅返回统计摘要（个人中心徽标）
//   POST   /api/v2/wrong-questions/{userId}/sample           随机抽取若干道未掌握的错题用于复习
//   DELETE /api/v2/wrong-questions/{userId}/{questionId}     从错题本移除单题
//   POST   /api/v2/wrong-questions/{userId}/{questionId}/master    标记已掌握
//   POST   /api/v2/wrong-questions/{userId}/{questionId}/unmaster  取消已掌握
//   POST   /api/v2/wrong-questions/{userId}/reset            清空整份错题本
// ---------------------------------------------------------------------------
void registerWrongQuestionRoutes(const AppContext &ctx)
{
    ...
    // 统计摘要
    app().registerHandler(
        "/api/v2/wrong-questions/{1}/summary",
        [ctx](const HttpRequestPtr &req,
              std::function<void(const HttpResponsePtr &)> &&callback,
              std::string userId) {
            handleRequest(req, std::move(callback), [&]() {
    ...
    // 随机抽样复习
    app().registerHandler(
        "/api/v2/wrong-questions/{1}/sample",
        [ctx](const HttpRequestPtr &req,
              std::function<void(const HttpResponsePtr &)> &&callback,
              std::string userId) {
            handleRequest(req, std::move(callback), [&]() {
    ...
    // 标记已掌握
    app().registerHandler(
        "/api/v2/wrong-questions/{1}/{2}/master",
        [ctx](const HttpRequestPtr &req,
              std::function<void(const HttpResponsePtr &)> &&callback,
              std::string userId,
              std::string questionId) {
    ...
    // 取消已掌握
    app().registerHandler(
        "/api/v2/wrong-questions/{1}/{2}/unmaster",
        [ctx](const HttpRequestPtr &req,
              std::function<void(const HttpResponsePtr &)> &&callback,
              std::string userId,
              std::string questionId) {
    ...
    // 清空错题本
    app().registerHandler(
        "/api/v2/wrong-questions/{1}/reset",
        [ctx](const HttpRequestPtr &req,
              std::function<void(const HttpResponsePtr &)> &&callback,
              std::string userId) {
            handleRequest(req, std::move(callback), [&]() {
