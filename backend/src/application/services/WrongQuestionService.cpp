                                           ...
                                           const Json::Value &scoreResult)
{
    ...
    for (const auto &questionKey : memberNames)
    {
        const auto &row = results[questionKey];
        const auto questionId = row.get("question_id", questionKey).asString();
        ...
        else if (status == "correct")
        {
            ...
            repository_.recordCorrect(userId, examId, questionKey);
        }
        // unanswered 不动
    }
}
