...
const Json::Value &examData) const
{
    ...
    const auto readSubmittedAnswer = [&](int sectionIndex, const std::string &questionId) {
        auto userAnswer = normalizeAnswer(answers[questionId]);
        if (!userAnswer.empty())
        {
            return userAnswer;
        }
        return normalizeAnswer(answers[std::to_string(sectionIndex) + ":" + questionId]);
    };

    const auto scoreQuestion = [&](int sectionIndex, const Json::Value &question) {
        const auto questionId = question.get("id", "").asString();
        const auto scoreKey = std::to_string(sectionIndex) + ":" + questionId;
        if (questionId.empty() || scoredQuestionIds.count(scoreKey) > 0)
        {
            return;
        }
        ...
        scoredQuestionIds.insert(scoreKey);
        ++totalQuestions;

        const auto userAnswer = readSubmittedAnswer(sectionIndex, questionId);
    ...
    for (const auto &section : examData["exam_info"]["sections"])
    {
        ...
        {
            scoreQuestion(sectionIndex, question);
        }
